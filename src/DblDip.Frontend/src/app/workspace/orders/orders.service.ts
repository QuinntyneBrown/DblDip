...
export class OrdersService {
  ...
  public getById(options: { orderId: number }): Observable<Order> {
  ...
  public save(options: { order: Order }): Observable<{ orderId: number }> {
    return this._client.post<{ orderId: number }>(`${this._baseUrl}api/orders`, { order: options.order });
  }  
}
